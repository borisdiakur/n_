...
var argsSpec = require('yargs')
  ...
  .option('history-path', {
    ...
    alias: ['f', 'filename', 'history', 'history-file'],
    string: true,
  ...
  .coerce('prompt', prompt => {
    for (var [key, color] of Object.entries(prompt.color)) {
...
var argsDefaults = argsSpec.parse([])
...
function injectLodashIntoRepl (server, lodashes, promptConfig) {
  ...
  var { help: helpColor, flavor: flavorColor } = promptConfig.color

  var setLodash = flavor => repl => {
  ...
  var clear = typeof server.clearBufferedCommand === 'function' ? server => server.clearBufferedCommand() : server => { server.bufferedCommand = '' }
...
function wrapRepl (args) {
  var completeArgs = Object.assign({}, argsDefaults, args)
  var promptConfig = {
    ...completeArgs.prompt,
    makePrompt (flavor) {
  ...
  var initialPrompt = promptConfig.makePrompt()
  var customReplServer = completeArgs.replServer instanceof repl.REPLServer

  var server = customReplServer ? completeArgs.replServer : repl.start({
